...
namespace game
{
    ...
    enum { BNC_GRENADE, BNC_GIBS, BNC_DEBRIS, BNC_DEBRISHOT, BNC_BARRELDEBRIS, BNC_SHELLRIFLE, BNC_SHELLPISTOL, BNC_SHELLSHOTGUN };
    ...
    void newbouncer(const vec &from, const vec &to, bool local, int id, fpsent *owner, int type, int lifetime, int speed, entitylight *light = NULL)
    {
        ...
        switch(type)
        {
            ...
            case BNC_DEBRIS: case BNC_BARRELDEBRIS: case BNC_DEBRISHOT: bnc.variant = rnd(4); break;
            case BNC_GIBS: bnc.variant = rnd(3); break;

            case BNC_SHELLRIFLE: bnc.collidetype = COLLIDE_ELLIPSE_PRECISE; break;
            case BNC_SHELLPISTOL: bnc.collidetype = COLLIDE_ELLIPSE_PRECISE; break;
            case BNC_SHELLSHOTGUN: bnc.collidetype = COLLIDE_ELLIPSE_PRECISE; break;
        }
    ...
    VARP(gibsounds, 0, 1, 1);
    //HVARFR(bloodcolour, 0, 0x60FFFF, 0xFFFFFF);
        ...
        switch (b->bouncetype)
        {
            case BNC_GIBS:
                if (b->bounces < 2)
                    if (gibsounds != 0)
                    {
                        playsound(S_GIBBOUNCE, &b->o);
                    }
                    b->bounces++;
                    adddecal(DECAL_BLOOD, vec(b->o).sub(vec(surface).mul(b->radius)), surface, 3.0f, bvec(0x60, 0xFF, 0xFF), rnd(4));
                    return;

            case BNC_GRENADE:
                if (b->bounces < 4)
                    playsound(S_GRNBOUNCE, &b->o);
                    b->bounces++;
                    return;

            case BNC_SHELLRIFLE:
                if (b->bounces < 1)
                    playsound(S_SHBCRIFLE, &b->o);
                    b->bounces++;
                    return;

            case BNC_SHELLPISTOL:
                if (b->bounces < 1)
                    playsound(S_SHBCPISTOL, &b->o);
                    b->bounces++;
                    return;

            case BNC_SHELLSHOTGUN:
                if (b->bounces < 1)
                    playsound(S_SHBCSHOTG, &b->o);
                    b->bounces++;
                    return;

            case BNC_DEBRIS: case BNC_DEBRISHOT: case BNC_BARRELDEBRIS:
                return;
    ...
    void updatebouncers(int time)
    {
        loopv(bouncers)
        {
            ...
            if (bnc.bouncetype == BNC_GIBS && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_BLOODMIST, 1, 500, pos, 0x10FFFF, 2.5f, 50, 0);
                regular_particle_splash(PART_BLOOD, 1, 350, pos, 0x60FFFF, 2.0f, 50, 0);
            }
            if(bnc.bouncetype == BNC_GRENADE && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKE, 1, 150, pos, 0x404040, 2.4f, 50, 0);
            }
            if (bnc.bouncetype == BNC_DEBRIS && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKE, 1, 150, pos, 0x404040, 1.2f, 50, 0);
            }
            if (bnc.bouncetype == BNC_DEBRISHOT && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKE, 1, 150, pos, 0x404040, 1.2f, 50, 0);
                regular_particle_splash(PART_SPARK, 1, 150, pos, 0xB49B4B, 1.0f, 50, 0);
            }
            if (bnc.bouncetype == BNC_BARRELDEBRIS && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKE, 1, 150, pos, 0x404040, 1.2f, 50, 0);
            }

            if (bnc.bouncetype == BNC_SHELLRIFLE && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKEADD, 1, 150, pos, 0x303030, 0.5f, 50, 0);
            }
            if (bnc.bouncetype == BNC_SHELLPISTOL && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKEADD, 1, 150, pos, 0x303030, 0.5f, 50, 0);
            }
            if (bnc.bouncetype == BNC_SHELLSHOTGUN && bnc.vel.magnitude() > 50.0f)
            {
                vec pos = bnc.offsetpos();
                regular_particle_splash(PART_SMOKEADD, 1, 150, pos, 0x303030, 0.5f, 50, 0);
            }
    ...
    void damageeffect(int damage, fpsent *d, bool thirdperson)
    {
        ...
        if(blood) particle_splash(PART_BLOODMIST, 1, 1000, p, 0x10FFFF, 10.0f, 150, 0);
        if(thirdperson)
        {
    ...
    void spawnbouncerdirspeed(const vec& p, const vec& dir, fpsent* d, int type, int vel, int lifetime, entitylight* light = NULL)
    {
        vec to(dir.x, dir.y, dir.z);
        if (to.iszero()) to.z += 1;
        to.normalize();
        to.add(p);
        newbouncer(p, to, true, 0, d, type, lifetime, vel, light);
    }

    VARP(gibamount, 1, 16, 256);
    ...
    void gibeffect(int damage, const vec &vel, fpsent *d)
    {
        ...
        particle_splash(PART_BLOODMIST, 1, 1000, from, 0x10FFFF, 10.0f, 150, 0);

        loopi(min(damage/25, 40)+gibamount) spawnbouncer(from, vel, d, BNC_GIBS);
    }
    ...
    VARP(distantexplosions, 0, 1, 1);

    void explode(bool local, fpsent *owner, const vec &v, dynent *safe, int damage, int gun)
    {
        particle_splash(PART_SPARK, 200, 150, v, 0xB49B4B, 1.0f, 500, 40.0f);
        particle_splash(PART_SMOKE, 5, 300, v, 0x404040, 25.0f, 150, 20.0f);
        particle_splash(PART_SMOKEADD, 5, 300, v, 0x202020, 25.0f, 150, 40.0f);

        playsound(gun != GUN_GL ? S_RLHIT : S_FEXPLODE, &v);
        if (distantexplosions != 0)
        {
            playsound(gun != GUN_GL ? S_DISTANTEXP : S_DISTANTGREN, &v, NULL, 0, 0, 0, -1, 4096 * 4);
        }
        ...
        if(gun==GUN_RL) debrisorigin.add(vec(debrisvel).mul(16));
        if(gun==GUN_GL) debrisorigin.add(vec(debrisvel).mul(8));
        if(numdebris)
        {
            entitylight light;
            ...
            loopi(numdebris/2)
                spawnbouncer(debrisorigin, debrisvel, owner, gun==GUN_BARREL ? BNC_BARRELDEBRIS : BNC_DEBRISHOT, &light);
        }
        if(!local) return;
    ...
    void projsplash(projectile &p, vec &v, dynent *safe, int damage)
    {
        if(guns[p.gun].part)
        {
            particle_splash(PART_SPARK, 100, 200, v, 0xB49B4B, 0.24f, 500, 2);
            ...
            case GUN_RL:
                loopv(projs)
                {
                    ...
                    if(p.gun == gun && p.owner == d && p.id == id && !p.local)
                    {
                        ...
                        particle_splash(PART_SPARK, 200, 150, pos, 0xB49B4B, 1.0f, 500, 40.0f);
                        particle_splash(PART_SMOKE, 5, 300, pos, 0x404040, 25.0f, 150, 20.0f);
                        projs.remove(i);
                        break;
                    }
                }
    ...
    VARP(distantguns, 0, 1, 1);
    VARP(ricochets, 0, 1, 1);

    VARP(gunshells, 0, 1, 1);
    VARP(shelllifetime, 0, 5000, 10000);
    ...
    {
        vec side = vec((d->yaw + 90) * RAD, 0.0f), trans = vec(0, 0, 0);
        vec directionforward = vec(d->yaw * RAD, 0.0f), directionside = vec((d->yaw + 90) * RAD, 0.0f);
        ...
        switch(gun)
        {
            ...
            case GUN_SG:
            {
                ...
                //vec offset2(offset.mul(viewmatrix));
                int speed(64.0f);

                float dist = d->muzzle.dist(to);
                vec up = to;
                up.z += dist / 8;
                //up.add(offset2);
                ...
                if(muzzleflash && d->muzzle.x >= 0)
                    ...
                    if (gunshells != 0)
                    {
                        newbouncer(d->muzzle, up, local, id, d, BNC_SHELLSHOTGUN, shelllifetime, speed);
                    }
                    if(distantguns != 0)
                    {
                        playsound(S_DISTANTSG, &d->o, NULL, 0, 0, 0, -1, 4096);
                    }
                    particle_flare(d->muzzle, d->muzzle, 65, PART_MUZZLE_FLASH3, 0xFFFFFF, 2.75f, d);
                    particle_splash(PART_SMOKEADD, 1, 500, d->muzzle, 0x404040, 8.0f, 150, 0.0f);
                    particle_splash(PART_SPARK, 20, 250, d->muzzle, 0xB49B4B, 0.24f);
                loopi(guns[gun].rays)
                {
                    //playsound(S_RICOCHET, &rays[i], NULL, 0, 0, 1, -1);
                    
                    particle_splash(PART_SPARK, 20, 250, rays[i], 0xB49B4B, 0.24f);
                    particle_splash(PART_SMOKEADD, 1, 500, rays[i], 0x303030, 3.0f, 150, -0.5f);
            ...
            case GUN_CG:
            {
                ...
                if (ricochets != 0)
                {
                    playsound(S_RICOCHET, &to);
                }
                particle_splash(PART_SPARK, 200, 250, to, 0xB49B4B, 0.24f);
                particle_splash(PART_SMOKEADD, 1, 500, to, 0x303030, 3.0f, 150, -0.5f);
                ...
                if (muzzleflash && d->muzzle.x >= 0)
                    ...
                    if (gunshells != 0)
                    {
                        newbouncer(d->muzzle, up, local, id, d, BNC_SHELLRIFLE, shelllifetime, speed);
                    }
                    if (distantguns != 0)
                    {
                        playsound(S_DISTANTMG, &d->o, NULL, 0, 0, 0, -1, 4096);
                    }
                    particle_flare(d->muzzle, d->muzzle, 50, PART_MUZZLE_FLASH1, 0xFFFFFF, 2.25f, d);
                    particle_splash(PART_SMOKEADD, 1, 500, d->muzzle, 0x404040, 5.0f, 150, 0.0f);
            ...
            case GUN_PISTOL:
            {
                ...
                if (muzzleflash && d->muzzle.x >= 0)
                    ...
                    if (gunshells != 0)
                    {
                        newbouncer(d->muzzle, up, local, id, d, BNC_SHELLPISTOL, shelllifetime, speed);
                    }
                    if(distantguns != 0)
                    {
                        playsound(S_DISTANTPIS, &d->o, NULL, 0, 0, 0, -1, 4096);
                    }
                    particle_flare(d->muzzle, d->muzzle, 65, PART_MUZZLE_FLASH1, 0xFFFFFF, 1.25f, d);
                    particle_splash(PART_SMOKEADD, 1, 500, d->muzzle, 0x606060, 5.0f, 150, 0.0f);

                if(!local) adddecal(DECAL_BULLET, to, vec(from).sub(to).safenormalize(), 2.0f);

                if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 15, vec(0.5f, 0.375f, 0.25f), 100, 100, DL_FLASH, 0, vec(0, 0, 0), d);
                break;
            }
            ...
            case GUN_RL:
                if(muzzleflash && d->muzzle.x >= 0)
                    if (distantguns != 0)
                    {
                        playsound(S_DISTANTRL, &d->o, NULL, 0, 0, 0, -1, 4096);
                    }
                    particle_flare(d->muzzle, d->muzzle, 75, PART_MUZZLE_FLASH2, 0xFFFFFF, 3.0f, d);
                    particle_splash(PART_SMOKEADD, 1, 500, d->muzzle, 0x404040, 8.0f, 150, 0.0f);
            case GUN_FIREBALL:
            case GUN_ICEBALL:
            case GUN_SLIMEBALL:
            ...
            case GUN_GL:
            {
                ...
                if(muzzleflash && d->muzzle.x >= 0)
                    if (distantguns != 0)
                    {
                        playsound(S_DISTANTGL, &d->o, NULL, 0, 0, 0, -1, 4096);
                    }
                particle_flare(d->muzzle, d->muzzle, 75, PART_MUZZLE_FLASH2, 0xFFFFFF, 1.5f, d);
                    particle_splash(PART_SMOKEADD, 1, 500, d->muzzle, 0x404040, 10.0f, 150, 0.0f);
            ...
            case GUN_RIFLE:
                ...
                if(muzzleflash && d->muzzle.x >= 0)
                    ...
                    if (distantguns != 0)
                    {
                        playsound(S_DISTANTRIF, &d->o, NULL, 0, 0, 0, -1, 4096);
                    }
                    particle_flare(d->muzzle, d->muzzle, 65, PART_MUZZLE_FLASH3, 0xFFFFFF, 1.25f, d);
                    particle_splash(PART_SMOKEADD, 1, 500, d->muzzle, 0x404040, 5.0f,150,0.0f);
    ...
    static const char * const debrishotnames[4] = { "debris/debris01", "debris/debris02", "debris/debris03", "debris/debris04" };
    ...
    static const char * const shellriflenames[1] = { "shells/rifleshell" };
    static const char * const shellpistolnames[1] = { "shells/pistolshell" };
    static const char * const shellshotgunnames[1] = { "shells/shotgunshell" };
    ...
    void preloadbouncers()
    {
        ...
        loopi(sizeof(debrishotnames) / sizeof(debrishotnames[0])) preloadmodel(debrishotnames[i]);
        loopi(sizeof(barreldebrisnames)/sizeof(barreldebrisnames[0])) preloadmodel(barreldebrisnames[i]);

        loopi(sizeof(shellriflenames) / sizeof(shellriflenames[0])) preloadmodel(shellriflenames[i]);
        loopi(sizeof(shellpistolnames) / sizeof(shellpistolnames[0])) preloadmodel(shellpistolnames[i]);
        loopi(sizeof(shellshotgunnames) / sizeof(shellshotgunnames[0])) preloadmodel(shellshotgunnames[i]);
    }
        ...
        loopv(bouncers)
        {
            ...
            else
            {
                ...
                switch(bnc.bouncetype)
                {
                    ...
                    case BNC_DEBRISHOT: mdl = debrishotnames[bnc.variant]; break;
                    case BNC_BARRELDEBRIS: mdl = barreldebrisnames[bnc.variant]; break;

                    case BNC_SHELLRIFLE: mdl = shellriflenames[bnc.variant]; break;
                    case BNC_SHELLPISTOL: mdl = shellpistolnames[bnc.variant]; break;
                    case BNC_SHELLSHOTGUN: mdl = shellshotgunnames[bnc.variant]; break;
                    default: continue;
                }
