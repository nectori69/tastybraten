...
namespace game
{
    ...
    void killed(fpsent *d, fpsent *actor)
    {
        ...
        if(actor->type==ENT_AI)
            ...
            conoutf(contype, "\f2%s took the easy way out...%s", dname, d==player1 ? "" : "");
    ...
    void clientdisconnected(int cn, bool notify)
    {
        ...
        fpsent *d = clients[cn];
        if(!d) return;
        if(notify && d->name[0]) conoutf("\f7%s\f4 has departed...", colorname(d));
        removeweapons(d);
    ...
    void drawgameclock(int w, int h)
    {
        ...
        if (secs == 0 && round_start == false)
        {
            playsound(S_GAMESTART);
            round_start = true;
        }

        if (secs <= 10 && secs > 0 && mins == 0)
        {
            if (last_secs != secs)
            {
                playsound(S_MENUTIMER);
                last_secs = secs;
            }
        }

        if (secs == 0 && mins == 0 && round_over == false)
        {
