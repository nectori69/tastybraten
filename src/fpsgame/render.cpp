...
{      
    ...
    //VARP(swaystyle, 0, 1, 2);
    FVAR(swaystep, 1, 35.0f, 100);
    FVAR(swayside, 0, 0.04f, 1);
    FVAR(swayup, -1, 0.05f, 1);

    FVAR(camswaydecay, 0.1f, 0.996f, 0.9999f);
    FVAR(camswayinertia, -1.0f, 0.04f, 1.0f);
    FVAR(camswaymaxinertia, 0.0f, 15.0f, 1000.0f);
    ...
    void drawhudmodel(fpsent *d, int anim, float speed = 0, int base = 0)
    {
        ...
        if (lastmillis != lastsway)
        {
            ...
            cameravelocity.mul(powf(camswaydecay, curtime));
            cameravelocity.add(vec2(camerarotation).mul(camswayinertia));
            cameravelocity.clamp(-camswaymaxinertia, camswaymaxinertia);
        ...
        sway.mul(swayside * cosf(steps));
        sway.z = swayup * (cosf(steps) * sinf(steps) - 1);
        ...
        rendermodel(NULL, gunname, anim, sway, testhudgun ? 0 : d->yaw+90 + cameravelocity.x, testhudgun ? 0 : d->pitch + cameravelocity.y + (vel.z*-0.03), MDL_LIGHT|MDL_HUD, interp, a, base, (int)ceil(speed));
